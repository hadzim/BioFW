...
class Plotdata
{
	...
	friend std::ostream & operator << (std::ostream & out, const Plotdata & pd);
	friend std::istream & operator >> (std::istream & in, Plotdata & pd);
...
public:
	...
	inline Plotdata(size_t s): data(s), userFunction(0),
                               userBinFunction(0){}
	inline Plotdata(std::vector<double> d):  data(d),
    ...
    static void colorChange(Plotdata &x, Plotdata &y, Color colour);
